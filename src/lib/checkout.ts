...
// Note: guest client-side persistence flows were removed. This module no longer
// persists intended purchases or checkout state to client storage.
...
export async function openSingleCheckout(opts?: SingleCheckoutOpts) {
	...
	if (!user) {
		// Anonymous users can open guest checkout; server-side webhook will
		// associate purchases with accounts when possible. No client persistence.
	...
	// Use consolidated PRO price id for single/premium flows
	const singlePriceId = process.env.NEXT_PUBLIC_PADDLE_PRICE_PRO || process.env.NEXT_PUBLIC_PADDLE_PRICE_SINGLE;
	...
	// Note: we no longer track `inCheckout` in client storage.
...
export async function openTierCheckout(tierId: string, priceId?: string) {
	...
	if (!user) {
		...
		if (typeof window !== 'undefined') {
			// Do not persist intendedPurchase to client storage; require sign-in
			// and start the checkout from the authenticated flow.
	...
	// Prefer explicit priceId, otherwise use consolidated PRO id
	const priceToUse = priceId || process.env.NEXT_PUBLIC_PADDLE_PRICE_PRO || process.env.NEXT_PUBLIC_PADDLE_PRICE_PREMIUM;
	...
	// No client-side `inCheckout` marker is set.
